...
describe("Adding and deleting items from cart.", function () {
  ...
  it("Check the information in the cart.", async function () {
    const product = new Product(this.productName, this.productPrice, this.productId, this.productSize, this.productUrl, this.cartUrl, this.orderingUrl);
    ...
    await productPage.openOrdering();
    
    const productPriceInCart = await productPage.getProductPriceInCart();
    expect(productPriceInCart).to.equal('8 499 руб.');
